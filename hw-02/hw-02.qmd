...
```{r}
# plotting categorical bmi
chs <- chs %>% 
  mutate(obesity_level = factor(obesity_level, levels = c("underweight", "normal", "overweight", "obese")))

ggplot(chs) +
  geom_bar(mapping = aes(x = obesity_level, fill = smoke_gas_exposure), position = "stack") +
  scale_fill_brewer(palette = "Set2") +
  labs(title = "Obesity Level by Smoke/Gas Exposure",
       x = "Obesity Level",
       y = "Count",
       fill = "Smoke/Gas Exposure")
```

This barchart of BMI/obesity level by smoke/gas exposure shows the number of children who are exposed to each category of smoke/gas exposure in each obesity level category. We can see that most children with gas stoves exposure fall under normal BMI, with overweight and obese categories being higher than underweight. In each category, the number of children with exposures increases with each obesity level except for normal which has the most counts for all categories of obesity.
